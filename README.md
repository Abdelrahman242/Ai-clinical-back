...
يستخدم الـAPI JWT بمدة افتراضية 24 ساعة قابلة للتعديل عبر `ACCESS_TOKEN_EXPIRE_MINUTES`. التوكنات الجديدة تعتمد على `user_id` الثابت داخل `sub` بدل `username`، لذلك تغيير اسم المستخدم لا يُسقط الجلسة الحالية. يتم قبول التوكنات القديمة التي كانت تعتمد على username للتوافق، لكنها ستحتاج إلى تسجيل دخول جديد بعد تغيير الاسم.

تحديث الملف الشخصي متاح عبر `PUT /api/v1/auth/me` و`PATCH /api/v1/auth/me`، ويقبل `username` أو `name`. بعد تحديث الاسم أو كلمة المرور، يعيد endpoint توكنًا جديدًا داخل `access_token` حتى يستطيع العميل استبدال أي توكن قديم. Endpoint `POST /api/v1/auth/logout` يسجل `jti` في جدول `revoked_tokens` داخل Supabase، وعلى الواجهة حذف التوكن المحلي أيضًا.

في بيئة الإنتاج يجب ضبط `JWT_SECRET_KEY` بشكل ثابت وقوي. إذا تغير هذا المتغير أو لم يكن مضبوطًا واعتمد التطبيق على القيمة الافتراضية، ستصبح التوكنات القديمة غير صالحة بعد إعادة التشغيل أو النشر.
